...
from hw_edits import _procrustes_hw as procrust
...
from hw_edits.srm_hw_stats import SRM, DetSRM
from hw_edits.utils_hw_stats import attempt_to_import, check_square_numpy_matrix
...
def align(data, method='deterministic_srm', n_features=None, axis=0,
    ...
    if method in ['deterministic_srm', 'probabilistic_srm']:
        ...
        print("padding rows with different sizes with zeros")
        ...
        print("Creating initial average template", len(m))
        for i, x in enumerate(m):
            ...
            if i == 0:
                ...
                print("complete procruste", i)
                template += trans

        template /= len(m)

        print("creating new common template")
        ...
        for i, x in enumerate(m):
            ...
            common += trans
        common /= len(m)
        print("aligning to a new template")
    ...
    print("calculating Intersubject correlation on aligned components")
    if n_features is None:
